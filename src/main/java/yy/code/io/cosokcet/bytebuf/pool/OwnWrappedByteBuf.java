...
public class OwnWrappedByteBuf extends ByteBuf {

    private static final InternalLogger LOGGER = InternalLoggerFactory.getInstance(OwnWrappedByteBuf.class);
    ...
    /**
     *我们使用一部分的,不追踪的directBuf,然后使用一部分的
     * @throws Throwable
     */
  //  @Override
...
//            LOGGER.error("global directBufPool There was a leak , should not call this method");
//        }

        //cas 计数一下泄露的数量
//        ownPool.incrementLeakCount();
//            ByteBuf buf = this.buf;
//            OwnWrappedByteBuf ownWrappedByteBuf = new OwnWrappedByteBuf(buf);
//            ownPool.returnBuf(ownWrappedByteBuf);
//            if (LOGGER.isErrorEnabled()) {
//                LOGGER.error("do the reuse");
//            }

        //拯救措施没有起作用
//        if (LOGGER.isErrorEnabled()) {
//            LOGGER.error("not do the reuse");
//        }
    //}

    int count = 1;
    protected final ByteBuf buf;
    protected ConcurrentGroupByteBufPool ownPool;


    void setOwnPool(ConcurrentGroupByteBufPool ownPool) {
        this.ownPool = ownPool;
    }

    protected OwnWrappedByteBuf(ByteBuf buf) {
        if (buf == null) {
